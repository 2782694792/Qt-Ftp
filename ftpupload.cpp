...
void FtpUpload::InitWidget()
{
    ...
    connect(ui.minButton,SIGNAL(clicked()),this,SLOT(showMinimized()));         //最小化
...
void FtpUpload::on_pushButton_Login_clicked()
{
    ...
    else
    {
        ...
        if(!ip.isEmpty() && !port.isEmpty() && !userName.isEmpty() && !psw.isEmpty())
        {
            ...
            int ret2 = m_ftp->login(userName,psw);//登陆服务器;
        }
        else
        {
...
void FtpUpload::S_commandFinish(int tmp, bool en)
{
    ...
    if(m_ftp->currentCommand() == QFtp::ConnectToHost)
    {
        if(en)
        {
            ...
            ui.textEdit->append(QString::fromLocal8Bit("连接服务器出现错误：%1").arg(m_ftp->errorString()));
        }
        else
        {
    ...
    if (m_ftp->currentCommand() == QFtp::Login)
    {
        ...
        if(en)
        {
            ...
            ui.textEdit->append(QString::fromLocal8Bit("登录出现错误：%1").arg(m_ftp->errorString()));
        }
        else
        {
    ...
    if (m_ftp->currentCommand() == QFtp::Get)
    {
        if(en)
        {
            ui.textEdit->append(QString::fromLocal8Bit("下载出现错误：%1").arg(m_ftp->errorString()));

            qDebug() << (QString::fromLocal8Bit("下载出现错误：%1").arg(m_ftp->errorString()));
        }
        else
        {
    ...
    else if(m_ftp->currentCommand() == QFtp::Put)
    {
        if(en)
        {
            myMessageOutput(QString::fromLocal8Bit("上传出现错误：%1").arg(m_ftp->errorString()));
            ui.textEdit->append(QString::fromLocal8Bit("上传出现错误：%1").arg(m_ftp->errorString()));
        }
        else
        {
            //if(m_loop.isRunning())
...
void FtpUpload::on_uploadButton_clicked()
{
    ...
    if (QFile::exists(fileName))
    {
        ...
        qDebug() << QString::fromLocal8Bit("文件%1的普通上传... ...").arg(fileName);
...
void FtpUpload::GetTime()
{
    ...
    if (current_hour == (set_hour + 1) && (60 - current_minute) == 2)
    {
        ...
        myMessageOutput(QString::fromLocal8Bit("当前时间：%1:%2:%3").arg(current_hour).arg(current_minute).arg(current_second));
        ScheduledUP();
        m_timer->start(60000);
...
void FtpUpload::fileRename()
{
    ...
    {
        QMessageBox::information(this, QString::fromLocal8Bit("保存"), QString::fromLocal8Bit("<font size='6' color='#008ed5'>文件夹不能重命名！</font>"), QMessageBox::Ok);
        return;
    }
    bool ok;
    QString newName = QInputDialog::getText(this, QString::fromLocal8Bit("重命名"),
                                            QString::fromLocal8Bit("新文件名："),
...
{
    bool ok;
    QString folderName = QInputDialog::getText(this, QString::fromLocal8Bit("新建文件夹"),
                                               QString::fromLocal8Bit("新文件夹名："),
                                               QLineEdit::Normal, QString::fromLocal8Bit(""),
                                               &ok,Qt::WindowCloseButtonHint);
...
void FtpUpload::Batch_upload(QString DirPath)
{
    ...
    if(dir.exists())
    {
        ...
        for(auto it =  fileLst.begin(); it != end_it; it++)
        {
            ...
            ui.textEdit->append(QString::fromLocal8Bit("当前正在上传:") + it->fileName());
            m_ftp->put(file,toSpecialEncoding(it->fileName()));
            myMessageOutput(QString::fromLocal8Bit("当前正在上传:") + it->filePath());
            //			m_loop.exec();
        }
        ui.progressBar->setValue(0);
...
void FtpUpload::Batch_upload2(QString DirPath)
{
    ...
    if(dir.exists())
    {
        ...
        for(auto it =  fileLst.begin(); it != end_it; it++)
        {
            ...
            QString _createTime = create.toString("yyyy-MM-dd");	//获取文件创建时间
            QDateTime currentTime = QDateTime::currentDateTime();
            QString currentTimeStr = currentTime.toString("yyyy-MM-dd");			//获取当前时间
            QString beforeDayTimeStr = currentTime.addDays(-1).toString("yyyy-MM-dd");  //获取前一天时间
